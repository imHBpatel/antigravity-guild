...
function auditFileContent(filePath, projectDir) {
  ...
  lines.forEach((line, idx) => {
    ...
    const isDocOrData = ext === '.md' || ext === '.json' || ext === '.yaml' || ext === '.yml' || ext === '.txt';
    const trimmed = line.trim();
    const isComment = trimmed.startsWith('//') || trimmed.startsWith('#') || trimmed.startsWith('*') || trimmed.startsWith('/*');
    const isConsoleLogging = /console\.(log|error|warn|info)\(/.test(line);

    // Check for raw eval (executable code only, exclude docs/comments/logging)
    if (!isDocOrData && !isComment && !isConsoleLogging && /\beval\s*\(/.test(line) && !relPath.includes('tests') && !relPath.includes('security_auditor')) {
      issues.push({
        file: relPath,
        line: lineNum,
        severity: 'HIGH',
    ...
    // Check for insecure SQL string concatenation (executable code only, exclude docs/comments/logging)
    if (!isDocOrData && !isComment && !isConsoleLogging && /(?:SELECT|INSERT|UPDATE|DELETE)\s+.*?\+\s*[A-Za-z0-9_]+/i.test(line) && !relPath.includes('tests') && !relPath.includes('security_auditor')) {
      issues.push({
        file: relPath,
        line: lineNum,
        severity: 'HIGH',
