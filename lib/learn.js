...
function recordLesson(insight, options = {}) {
  ...
  // Read existing content to check for exact duplicate and resolve conflicts
  let existingContent = '';
  ...
  if (fs.existsSync(filePath)) {
    ...
    // Detect and resolve contradictions/superseded historical rules
    const { detectContradictions, markLineSuperseded } = require('./conflict_resolver');
    const lines = existingContent.split(/\r?\n/);
    const existingEntries = lines
      .map((l, idx) => ({ text: l.trim(), lineNum: idx + 1 }))
      .filter(e => e.text.startsWith('-') || e.text.startsWith('*'));

    const conflictRes = detectContradictions(sanitized, existingEntries);
    if (conflictRes.hasConflict) {
      for (const conflict of conflictRes.conflicts) {
        const lineIdx = conflict.oldEntry.lineNum - 1;
        lines[lineIdx] = markLineSuperseded(lines[lineIdx], conflict.reason);
  ...
  const conflictNotice = resolvedConflicts.length > 0
    ? ` (Auto-superseded ${resolvedConflicts.length} older conflicting rule${resolvedConflicts.length > 1 ? 's' : ''})`
    : '';
  ...
  return {
    ...
    entry: formattedEntry,
    resolvedConflicts,
    message: `Recorded lesson into ${scope === 'team' ? 'team' : 'global'} memory (${filename})${conflictNotice}.`,
  };
}
