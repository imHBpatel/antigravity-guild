...
const { readMemoryBank, updateActiveTask } = require('./memory_bank');
const { searchMemoryVaults } = require('./hybrid_search');
const { resolveAllVaultConflicts } = require('./conflict_resolver');
...
const MCP_TOOLS = [
  ...
  },
  {
    ...
    description: 'Search across all global and team memory files using BM25 Okapi probabilistic ranking to retrieve the most relevant architectural rules, bugfixes, and conventions.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  {
    name: 'openguild_get_repo_map',
    description: 'Extract a token-efficient AST symbol map of the codebase (classes, interfaces, functions, methods, types) across JS/TS, Python, Go, and Rust without burning prompt context.',
    inputSchema: {
      type: 'object',
      properties: {
        projectPath: {
        ...
        query: {
          type: 'string',
          description: 'Optional symbol or filename search query to filter the map.',
        },
        maxTokens: {
          type: 'number',
          description: 'Max token budget for the returned map (default: 800).',
          default: 800,
        },
      },
    },
  ...
  {
    name: 'openguild_read_memory_bank',
    description: 'Read active task context, architectural patterns, and decision logs from the living memory bank in .openguild/.',
    inputSchema: {
      type: 'object',
      properties: {
        projectPath: {
        ...
        file: {
          type: 'string',
          description: 'Specific memory bank file to read (e.g. "active_task.md", "system_patterns.md", "decision_log.md"). Omit to read aggregated bank.',
        },
      },
    },
  },
  {
    name: 'openguild_update_active_task',
    description: 'Autonomously update the active engineering task context in .openguild/active_task.md to maintain task continuity, constraints, and progress across chat turns.',
    inputSchema: {
      type: 'object',
      properties: {
        goal: {
          type: 'string',
          description: 'Current active task goal or objective.',
        },
        status: {
          type: 'string',
          enum: ['in_progress', 'blocked', 'review', 'completed'],
          description: 'Current status of the task.',
        },
        nextSteps: {
          type: 'array',
          items: { type: 'string' },
          description: 'Updated list of next steps to execute.',
        },
        constraints: {
          type: 'array',
          items: { type: 'string' },
          description: 'Active constraints or user preferences to observe.',
        },
        notes: {
          type: 'string',
          description: 'Recent session notes, discoveries, or bugfix root causes.',
        },
        projectPath: {
          type: 'string',
          description: 'Directory path of the project. Defaults to current working directory.',
  ...
  {
    name: 'openguild_resolve_memory_conflicts',
    description: 'Audit global and team memory vaults to detect and resolve contradictory or superseded rules (e.g. tech stack migrations), marking obsolete entries to eliminate AI hallucinations.',
    inputSchema: {
      type: 'object',
      properties: {
        dryRun: {
          type: 'boolean',
          description: 'Preview conflicting and superseded rules without modifying files. Default: false.',
          default: false,
        },
      },
    },
  ...
  {
    name: 'openguild_council_sop_review',
    description: 'Execute a formal 4-stage Council Standard Operating Procedure (SOP) review across Architecture, Security (SAIF 2.0), QA Invariants, and Apple UX/DX.',
    inputSchema: {
      type: 'object',
      properties: {
        code: {
          type: 'string',
          description: 'Source code or git diff to review.',
        },
        filePath: {
          type: 'string',
          description: 'Optional file path to read and review from disk.',
        },
        intent: {
          type: 'string',
          description: 'The user story, feature specification, or intent of this code.',
        },
      },
    },
  },
  {
    name: 'openguild_pack_context',
    description: 'Package a project codebase into a token-budgeted, secret-scrubbed context bundle with an ASCII file tree for AI model consumption.',
    inputSchema: {
      type: 'object',
      properties: {
        projectPath: {
          type: 'string',
          description: 'Directory path to package. Defaults to current working directory.',
        },
        outputFile: {
          type: 'string',
          description: 'Optional output markdown file path to save the bundle.',
        },
        maxFiles: {
          type: 'number',
          description: 'Max number of files to bundle (default: 60).',
          default: 60,
        },
        maxTokens: {
          type: 'number',
...
function handleToolCall(name, args = {}) {
  ...
  switch (name) {
    ...
    case 'openguild_search_memory': {
      try {
        const result = searchMemoryVaults(memoryDirs, args.query || '');
...
`;
    ...
    case 'openguild_get_repo_map': {
      try {
        const targetDir = args.projectPath ? path.resolve(args.projectPath) : process.cwd();
        const result = generateRepoMap(targetDir, {
          query: args.query,
          maxTokens: args.maxTokens,
        });
        return { content: [{ type: 'text', text: result.mapMarkdown }] };
      } catch (err) {
        return { content: [{ type: 'text', text: `Failed to generate repo map: ${err.message}` }], isError: true };
      }
    }

    case 'openguild_read_memory_bank': {
      try {
        const targetDir = args.projectPath ? path.resolve(args.projectPath) : process.cwd();
        const result = readMemoryBank(targetDir, args.file);
        return {
          content: [{ type: 'text', text: result.text }],
          isError: !result.exists,
        };
      } catch (err) {
        return { content: [{ type: 'text', text: `Failed to read memory bank: ${err.message}` }], isError: true };
      }
    }

    case 'openguild_update_active_task': {
      try {
        const targetDir = args.projectPath ? path.resolve(args.projectPath) : process.cwd();
        const result = updateActiveTask(targetDir, {
          goal: args.goal,
          status: args.status,
        ...
        return { content: [{ type: 'text', text: `Failed to update active task: ${err.message}` }], isError: true };
      }
    }

    case 'openguild_resolve_memory_conflicts': {
      try {
        const vaultDirs = memoryDirs.map(m => m.dir);
        const result = resolveAllVaultConflicts(vaultDirs, { dryRun: args.dryRun });
        return { content: [{ type: 'text', text: result.markdown }] };
      } catch (err) {
        return { content: [{ type: 'text', text: `Conflict resolution failed: ${err.message}` }], isError: true };
      }
    }

    case 'openguild_council_sop_review': {
      try {
        const input = args.filePath || args.code || '';
        const result = runCouncilSop(input, { intent: args.intent, filename: args.filePath });
        return { content: [{ type: 'text', text: result.reportMarkdown }], isError: !result.passed };
      } catch (err) {
        return { content: [{ type: 'text', text: `Council SOP review failed: ${err.message}` }], isError: true };
      }
    }

    case 'openguild_pack_context': {
      try {
        const targetDir = args.projectPath ? path.resolve(args.projectPath) : process.cwd();
        const result = packCodebase(targetDir, {
          outputFile: args.outputFile,
          maxFiles: args.maxFiles,
          maxTokens: args.maxTokens,
        });
        return { content: [{ type: 'text', text: result.bundleMarkdown }] };
      } catch (err) {
        return { content: [{ type: 'text', text: `Context packaging failed: ${err.message}` }], isError: true };
      }
    }
