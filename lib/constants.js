...
// The 16-Mind Supreme Council Registry
const COUNCIL_MEMBERS = [
  ...
  { id: 'cognitive_analyst', icon: '🔮', title: 'Chief Cognitive Analyst & Requirements Oracle', domain: 'Latent requirement extraction, domain edge cases, product anticipation & user story fidelity.' },
  { id: 'agi_scientist', icon: '🔬', title: 'Principal AGI & Cognitive Architecture Scientist', domain: 'Multi-agent orchestration, neuro-symbolic reasoning, test-time compute scaling & cognitive loops.' },
  { id: 'self_healing', icon: '🧬', title: 'Autonomous Self-Healing & Evolution Specialist', domain: 'Runtime fault recovery, self-correcting agent loops, AST refactoring & automated regression patching.' },
  { id: 'knowledge_graph', icon: '📊', title: 'High-Dimensional Knowledge & Vector Graph Architect', domain: 'Semantic ontology trees, persistent vector indexing, hybrid graph-RAG & cognitive memory.' },
];

// Domain Presets
const PRESETS = {
  full: {
    name: 'Full 16-Mind Supreme Guild',
    roles: ['architect', 'ui_ux', 'security', 'staff_eng', 'qa', 'devops', 'google_cto', 'apple_cto', 'anthropic_safety', 'performance', 'database', 'product', 'cognitive_analyst', 'agi_scientist', 'self_healing', 'knowledge_graph'],
  },
  agi: {
    name: 'AGI & Autonomous Cognitive Systems',
    roles: ['architect', 'cognitive_analyst', 'agi_scientist', 'self_healing', 'knowledge_graph', 'anthropic_safety', 'google_cto', 'security', 'performance', 'qa', 'staff_eng'],
  },
  backend: {
  ...
  'ai-ml': {
    ...
    roles: ['architect', 'google_cto', 'performance', 'database', 'anthropic_safety', 'qa', 'staff_eng', 'knowledge_graph', 'agi_scientist'],
  },
};
...
`,
  'council_registry.md': `# The 16-Mind Supreme Council Registry
...
13. 🔮 Chief Cognitive Analyst & Requirements Oracle: Latent requirement extraction, domain edge cases & product anticipation.
14. 🔬 Principal AGI & Cognitive Architecture Scientist: Multi-agent orchestration, neuro-symbolic reasoning & cognitive loops.
15. 🧬 Autonomous Self-Healing & Evolution Specialist: Runtime fault recovery, self-correcting agent loops & AST refactoring.
16. 📊 High-Dimensional Knowledge & Vector Graph Architect: Semantic ontology trees, persistent vector indexing & hybrid graph-RAG.
`
};
...
function getBanner() {
  return `
${c.cyan}╔══════════════════════════════════════════════════════════════╗
║               ⚡ OMNIGUILD v${VERSION.padEnd(5)}: ALL-IN-ONE AI ENGINE        ║
║   16-Mind Council • BM25 Memory • AST Repo-Map • Invariants   ║
╚══════════════════════════════════════════════════════════════╝${c.reset}
`;
}
...
The All-In-One AI Developer Engine: 16-Mind Council, BM25 Memory, AST Repo-Map, Living Context & Invariants.

${c.bold}USAGE${c.reset}
  ${c.dim}$${c.reset} npx omniguild ${c.dim}[command] [options]${c.reset}
  ${c.dim}$${c.reset} npx antigravity-guild ${c.dim}[command] [options]${c.reset}
  ${c.dim}$${c.reset} npx openguild ${c.dim}[command] [options]${c.reset}

${c.bold}COMMANDS${c.reset}
  ${c.green}-u, --ui${c.reset}               Launch interactive Apple-grade terminal HUD dashboard
  ${c.green}--dashboard, --web${c.reset}   Launch local zero-dependency web visualizer dashboard
  ${c.green}--heal${c.reset}                 Run autonomous self-healing test runner and error diagnosis
  ${c.green}--graph${c.reset}                Letta L1/L2 memory pager & Cognee semantic knowledge graph
  ${c.green}--reproduce [issue]${c.reset}    Devin/SWE-agent hermetic reproducer test generator
  ${c.green}--apply <patch.diff>${c.reset}   Aider surgical SEARCH/REPLACE diff applicator
  ${c.green}--undo${c.reset}                Safe git rollback to previous checkpoint
  ${c.green}--context <@query>${c.reset}     Cursor/Claude Code smart @context expander (@symbol, @file, @git)
  ${c.green}--pr${c.reset}                   Generate executive Council PR description from git diff
  ${c.green}--commit [msg]${c.reset}         Synthesize standardized conventional commit with Council sign-off
  ...
  ${c.green}--review [file]${c.reset}      Execute formal 4-stage Council SOP multi-agent code review
  ${c.green}--pack [path]${c.reset}        Package codebase into token-budgeted markdown context bundle
  ${c.green}-q, --search <query>${c.reset} Probabilistic BM25 Okapi search across all memory vaults
  ${c.green}--resolve-conflicts${c.reset}  Audit and resolve contradictory/superseded memory rules
  ${c.green}-a, --analyze [vision]${c.reset}Auto-analyze project vision or workspace and synthesize engineering blueprint
  ${c.green}-m, --repomap [query]${c.reset} Generate token-efficient AST symbol map across JS/TS, Python, Go, and Rust
  ${c.green}--memory-bank${c.reset}        Inspect and scaffold living memory bank (.openguild/active_task.md)
  ${c.green}--tokens${c.reset}             Profile AI contract token consumption and measure context savings
  ${c.green}--verify [--fix]${c.reset}     Execute unified deterministic verification proof across all invariants
  ${c.green}--audit${c.reset}              Perform deep SAIF 2.0 security audit for secret leaks and vulnerabilities
  ${c.green}--setup-ci${c.reset}           Generate GitHub Actions CI workflow with 16-Mind Council checks
  ${c.green}--learn "<insight>"${c.reset}  Capture engineering lesson into memory with automated secret scrubbing
...
${c.bold}OPTIONS${c.reset}
  ${c.green}-p, --preset <name>${c.reset}  Apply domain preset (${c.bold}full${c.reset}, ${c.bold}agi${c.reset}, ${c.bold}backend${c.reset}, ${c.bold}web${c.reset}, ${c.bold}mobile${c.reset}, ${c.bold}ai-ml${c.reset})
