...
const { initMemoryBank } = require('./memory_bank');
 ...
 */
function initTeamMemory(projectDir = process.cwd(), dryRun = false) {
  ...
  if (!alreadyExists) {
    ...
    // Initialize living memory bank files as well
    initMemoryBank(projectDir, dryRun);

    console.log(`${c.green}✨ [Created]${c.reset} Team Memory Vault & Living Memory Bank at: ${c.dim}${teamDir}${c.reset}${dryRun ? ' (dry-run)' : ''}`);
    console.log(`   Committed to Git so all teammates share project memory automatically.`);
    return { path: teamDir, created: true };
  } else {
    // Ensure any missing memory bank files are lazily scaffolded non-destructively
    initMemoryBank(projectDir, dryRun);

    console.log(`${c.cyan}👥 [Connected]${c.reset} Team Memory Vault & Living Memory Bank at: ${c.dim}${teamDir}${c.reset}`);
